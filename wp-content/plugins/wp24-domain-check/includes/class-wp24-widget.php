...
class WP24_Domain_Check_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			'WP24 Domain Check',
			array(
				'description' => __( 'Display domaincheck form.', 'wp24-domaincheck' ),
			)
		);
	}
	...
	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Domain Check', 'wp24-domaincheck' );
		
		echo '<p>';
		echo '<label for="' . esc_attr( $this->get_field_id( 'title' ) ) . '">'. esc_attr_e( 'Title:', 'wp24-domaincheck' ) . '</label>';
